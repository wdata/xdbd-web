  ...
  this.propertyAlter = true;
	...
	if(property.haveHead){
      ...
      tmp+="<label title='"+(property.initLabelText||this_jobName)+"' "
		...
		this.onBtnWaitForClick=null//定时执行
		...
		case "ico_waitFor": if(This.onBtnWaitForClick!=null) This.onBtnWaitForClick();break;
			}
		});
	}
	var toolWidth=0;
	...
	this.$bgDiv.append("<div class='GooFlow_work' style='width:"+(width-12)+"px;height:"+(height)+"px;"+(property.haveHead? "":"margin-top:3px")+"'></div>");
	this.$workArea=$("<div class='GooFlow_work_inner' style='width:"+(width-12)+"px;height:"+(height-10)+"px'></div>")
...
///////////以下为有关画线的方法
	...
	reinitSize:function(width,height){
		...
		this.$draw.style.height = +h + "px";
		if(this.$group!=null){
			this.$group.css({height:h+"px",width:w-50+"px"});
		}
	},
